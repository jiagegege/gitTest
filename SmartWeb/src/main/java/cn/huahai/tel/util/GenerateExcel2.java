...
public class GenerateExcel2 {
	...
	public static byte[] writeExcel(List<Map<String, Object>> dataList, String modelPath, List<String> fields,
									HttpSession session, List<CriticalValueBase> setValue, int colors, int two) {
		...
		try {
			...
			Double serverCell = 0.0;
			Double HightCell = 0.0;
			Double NotGbCell = 0.0;
			Double disturbCell = 0.0;
			Double LinkCell = 0.0;
			for (int j = 0; j < dataList.size(); j++) {
				...
				dataMap = dataList.get(j);	
					...
					for (int i = 0; i < dataMap.size() - (dataMap.size() - fields.size()); i++) {
						...
						if(fields.get(i).equals("有业务小区数")) {
							serverCell += (Double) dataMap.get(datatype);
						}
						if(fields.get(i).equals("高用户小区数")) {
							HightCell += (Double) dataMap.get(datatype);
						}
						if(fields.get(i).equals("无服务小区数")) {
							NotGbCell += (Double) dataMap.get(datatype);
						}
						if(fields.get(i).equals("干扰小区数")) {
							disturbCell += (Double) dataMap.get(datatype);
						}
						if(fields.get(i).equals("断链小区数")) {
							LinkCell += (Double) dataMap.get(datatype);
						}
			...
			Row row = sheet.createRow(dataList.size() + 1);
			for (int i = 0; i < dataMap.size() - (dataMap.size() - fields.size()); i++) {
			//	List<Object> j = new ArrayList<Object>();
				Object[] j = null;
				if(two != 2 ) {
					 j = new Object[]{"汇总统计","","所有区域",serverCell,HightCell,NotGbCell,disturbCell,LinkCell}; 
				}else {
					 j = new Object[]{"汇总统计","所有区域",serverCell,HightCell,NotGbCell,disturbCell,LinkCell}; 
				}
				...
				if(j[i] instanceof Double) {
					cell.setCellValue((Boolean) j[i]);
				}else {
					cell.setCellValue((String) j[i]);
				}
	...
	public static <T> Map<String, Object> bean2Map(T source) throws IllegalAccessException {
		...
		for (Field field : sourceFiled) {
			...
			if (fieldName == null) {
//				if(field.getName().equals("有业务小区数")) {
//					
//					a = a+(Double) field.get(source);
//					i++;
//					
//				}
